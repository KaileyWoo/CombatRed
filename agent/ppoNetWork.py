...
class ActorCritic(nn.Module):
    ...
    def act(self, state):
        ...
        action = dist.sample()
        action = torch.clamp(action, -1, 1)  # [-max,max]
...
class PPO:
    def __init__(self, state_dim, action_dim, lr_actor, lr_critic, gamma, K_epochs, eps_clip,
        ...
        # 状态维度、动作维度、actor学习率、Critic学习率、奖励折扣系数、K步更新、裁剪系数、是否连续、初始化方差
    ...
    def update(self, episode):
        ...
        rewards = (rewards - rewards.mean()) / (rewards.std() + 1e-5) #1e-7
        ...
        for _ in range(self.K_epochs):
            ...
            # print('S_V:',state_values.detach().size())
            ...
            loss.mean().backward()
            if use_grad_clip:  # Trick 7: Gradient clip
        ...
        if use_lr_decay:  # Trick 6:learning rate Decay
    ...
    def save(self):
        # 保存神经网络
        torch.save(self.policy_old.state_dict(), log_dir + "ppo.pkl")

    def load(self):
        # 加载神经网络
        print("加载神经网络，路径：" + load_dir + "ppo.pkl")
        print("============================================================================================")
        self.policy_old.load_state_dict(torch.load(load_dir + "ppo.pkl", map_location=lambda storage, loc: storage))
        self.policy.load_state_dict(torch.load(load_dir + "ppo.pkl", map_location=lambda storage, loc: storage))